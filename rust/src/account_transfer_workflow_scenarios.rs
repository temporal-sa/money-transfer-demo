...
mod tests {
    ...
    #[test]
    fn scenario_adapters_export_expected_definitions() {
        let definitions = [
            AccountTransferHumanInLoop::definition(),
            AccountTransferAdvancedVisibility::definition(),
            AccountTransferRecoverableFailure::definition(),
            AccountTransferApiDowntime::definition(),
            AccountTransferInvalidAccount::definition(),
        ];
        let workflow_types = definitions
            .iter()
        ...
        for definition in definitions {
            assert_eq!(definition.signals, ["approveTransfer"]);
            assert_eq!(definition.queries, ["transferStatus"]);
            assert_eq!(definition.updates.len(), 1);
            assert_eq!(definition.updates[0].name, "approveTransferUpdate");
            assert!(definition.updates[0].has_validator);
        }
    }
}
